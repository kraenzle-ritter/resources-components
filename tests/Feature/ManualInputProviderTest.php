...
class ManualInputProviderTest extends TestCase
{
    ...
    public function test_manual_input_resource_lifecycle()
    {
        // Testdaten für manuelle Eingabe
        $provider = 'Manuell';
        $providerId = 'custom-123';
        $url = 'https://example.org/custom-resource';

        // 1. Modell erstellen
        $model = DummyModel::create(['name' => 'Test Manueller Input Model']);
        ...
        $this->assertTrue($model->resources->contains('provider_id', $providerId),
            "Die manuell eingegebene Resource wurde nicht erfolgreich hinzugefügt");
        ...
        $this->assertFalse($model->resources->contains('provider_id', $providerId),
            "Die manuell eingegebene Resource wurde nicht erfolgreich entfernt");
    }
}
